...
function progress(current, total, label) {
  ...
  process.stdout.write(
    `\r${colors.cyan}${label}: [${bar}] ${percentage}% (${current}/${total})${colors.reset}`,
  );
  ...
  for (let i = 0; i < breadth; i++) {
    ...
    switch (type) {
      ...
      case 1:
        ...
        break;
      case 2:
        obj[key] = Array.from({ length: randomInt(3, 7) }, () =>
          randomString(10),
        );
        break;
      case 3:
...
async function populateRedis() {
  ...
  try {
    ...
    for (let i = 0; i < 100; i++) {
      await client.set(
        `string:unicode:${i}`,
        generateUnicode() + randomString(50),
      );
      keysCreated++;
    }
    ...
    await client.set(
      specialStrings[2],
      `Single 'quotes' and "double quotes" and \`backticks\``,
    );
    await client.set(
      specialStrings[3],
    ...
    const veryLargeList = Array.from(
      { length: 10000 },
      (_, i) => `mega-item-${i}`,
    );
    ...
    for (let i = 0; i < 50; i++) {
      const jsonItems = Array.from({ length: 20 }, () =>
        JSON.stringify(generateJSON(2, 3)),
      );
    ...
    // Small sets
    for (let i = 0; i < 300; i++) {
      const members = Array.from({ length: randomInt(10, 50) }, () =>
        randomString(15),
      );
    ...
    // Sets with Unicode
    for (let i = 0; i < 30; i++) {
      const unicodeMembers = Array.from({ length: 20 }, () =>
        generateUnicode(),
      );
    ...
    for (let i = 0; i < 500; i++) {
      await client.hSet(`hash:user:${i}`, {
        ...
        country: ["USA", "UK", "Canada", "Germany", "France", "Japan"][
          randomInt(0, 5)
        ],
        created_at: new Date(
          Date.now() - randomInt(0, 86400000 * 365),
        ).toISOString(),
    ...
    for (let i = 0; i < 50; i++) {
      await client.hSet(`hash:json:${i}`, {
        ...
        data: JSON.stringify(
          Array.from({ length: 10 }, () => randomString(20)),
        ),
      });
      keysCreated++;
    }
    ...
    // ============================================================
    // 6.5. STREAMS - Redis Streams (XADD, XGROUP, XREAD)
    // ============================================================
    log("\n🌊 Creating STREAM keys...", "yellow");

    // Small streams
    for (let i = 0; i < 50; i++) {
      const streamKey = `stream:small:${i}`;
    ...
    log("Created 50 small streams", "cyan");

    // Large streams (1000 entries)
    for (let i = 0; i < 10; i++) {
      const streamKey = `stream:large:${i}`;
    ...
    log("Created 10 large streams (1000 entries each)", "cyan");

    // Streams with JSON payloads
    for (let i = 0; i < 20; i++) {
      const streamKey = `stream:json:${i}`;
      for (let j = 0; j < 50; j++) {
        await client.xAdd(streamKey, "*", {
          payload: JSON.stringify(generateJSON(3, 4)),
          source: "service-api",
        });
      }
      keysCreated++;
    }
    log("Created 20 streams with JSON payloads", "cyan");

    // Unicode & special characters in streams
    const unicodeStream = "stream:unicode:test";
    for (let i = 0; i < 30; i++) {
      await client.xAdd(unicodeStream, "*", {
        message: generateUnicode(),
        special: "newline\n tab\t quotes\"'",
      });
    }
    keysCreated++;
    log("Created stream with Unicode & special characters", "cyan");

    // Consumer group streams
    const groupStream = "stream:consumer:orders";
    const groupName = "order-processors";

    // Create stream entries
    for (let i = 0; i < 100; i++) {
      await client.xAdd(groupStream, "*", {
        order_id: `ORD-${1000 + i}`,
        amount: String(randomFloat(10, 500)),
        status: "created",
      });
    }

    // Create consumer group (MKSTREAM ensures stream exists)
    try {
      await client.xGroupCreate(groupStream, groupName, "0", {
        MKSTREAM: true,
      });
    } catch (err) {
      // Ignore BUSYGROUP error if re-running script
      if (!err.message.includes("BUSYGROUP")) throw err;
    }

    // Read some messages as consumers (creates PEL entries)
    ...
    keysCreated++;
    log("Created stream with consumer group & pending entries", "cyan");

    // Stream with TTL
    const ttlStream = "stream:ttl:test";
    for (let i = 0; i < 20; i++) {
      await client.xAdd(ttlStream, "*", {
        msg: `Expires soon ${i}`,
      });
    }
    await client.expire(ttlStream, 300); // 5 minutes
    keysCreated++;
    log("Created stream with TTL (5 minutes)", "cyan");
    ...
    if (remaining > 0) {
      log(
        `\n💪 Creating ${remaining} additional stress test keys...`,
        "yellow",
      );

      for (let i = 0; i < remaining; i++) {
        const type = i % 6; // Changed from 5 to 6 to include streams
        switch (type) {
          case 0:
            await client.set(`stress:string:${i}`, randomString(50));
            break;
          case 1:
            await client.rPush(`stress:list:${i}`, [
              randomString(20),
              randomString(20),
            ]);
            break;
          case 2:
            await client.sAdd(`stress:set:${i}`, [
              randomString(15),
              randomString(15),
            ]);
            break;
          case 3:
            await client.zAdd(`stress:zset:${i}`, [
              { score: i, value: randomString(10) },
            ]);
            break;
          case 4:
            await client.hSet(`stress:hash:${i}`, {
              field1: randomString(20),
              field2: randomString(20),
            });
            break;
          case 5:
            await client.xAdd(`stress:stream:${i}`, "*", {
              field1: randomString(20),
              field2: randomString(20),
              timestamp: String(Date.now()),
            });
    ...
    await client.set(
      "edge:special:key*with?chars[test]",
    ...
    log(
      "  • Strings: ~1,000+ (including JSON, Unicode, large values)",
      "white",
    );
    ...
    log("  • Streams: ~100+ (events, logs, sensor data, orders)", "white");
    ...
    log("  9. Test stream entry viewing and editing (stream:*)", "white");
